...
import { Wallet, TrendingUp, Droplets, Lightbulb, Loader2, Wrench, FileDown, AlertTriangle } from 'lucide-react';
...
const FinancialTab: React.FC<FinancialTabProps> = ({ state }) => {
  ...
  const kmRows = [
    ...
    { area: 'Manutenção', dep: state.reservasAtuais.manutencao + (state.saldosAcumulados.manutencao || 0), meta: 0, icon: <Wrench size={14} className="text-[#3B82F6]"/>, color: 'text-[#3B82F6]', detail: state.maintenancePlans },
  ];
  ...
  return (
    <div className="flex flex-col gap-[16px] animate-slide-up pb-8">
      ...
      {dashboard.deficitAcumulado > 0 && (
        <div className="bg-amber-500/10 border-2 border-amber-500 p-4 rounded-2xl flex items-center gap-3">
          <div className="bg-amber-500 text-white p-2 rounded-xl"><AlertTriangle size={20}/></div>
          <div>
            <h3 className="text-[12px] font-black text-amber-500 italic uppercase leading-none">Meta Acumulada (Déficit)</h3>
            <p className="text-[9px] font-bold text-amber-500/80 mt-1 uppercase">Você possui R$ {formatCurrency(dashboard.deficitAcumulado)} pendentes de dias anteriores. Ganhos atuais priorizarão este saldo.</p>
          </div>
        </div>
      )}
          ...
          <button 
            onClick={() => {
              ...
              if (val > 0) {
                ...
                input.value = '';
                // Removed alert to comply with AIS rules
              }
            }}
