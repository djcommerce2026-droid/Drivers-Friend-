...
import { Plus, Droplets, MapPin, CheckCircle, Settings2, Save, Edit2, AlertCircle } from 'lucide-react';
...
const FuelTab: React.FC<FuelTabProps> = ({ entries, onAdd, onAddStation, onUpdateVehicle, state, tankLevel }) => {
  ...
  const handleSave = () => {
    ...
    if (isNaN(valNum) || isNaN(priceNum) || isNaN(odoNum) || !data.stationName) {
      setFormError(true);
        ...
        <div className="fixed inset-0 bg-[#0F172A]/95 z-[3000] p-4 flex items-center justify-center backdrop-blur-sm">
          <div className="bg-[#1E293B] border border-[#334155] w-full max-w-xs p-6 rounded-3xl space-y-4 shadow-2xl animate-slide-up">
            ...
            <div className="space-y-4">
               ...
               <button onClick={handleSave} className={`w-full text-white py-3.5 rounded-2xl font-black italic shadow-xl text-[12px] uppercase tracking-wider transition-all ${formError ? 'bg-rose-500 animate-shake' : 'bg-[#3B82F6]'}`}>
                 {formError ? (
                   <span className="flex items-center justify-center gap-2"><AlertCircle size={16}/> Preencha tudo!</span>
                 ) : (
                   editingEntry ? 'Salvar alterações ✓' : 'Registrar abastecimento ✓'
                 )}
               </button>
