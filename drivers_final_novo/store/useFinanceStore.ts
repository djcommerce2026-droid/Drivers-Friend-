...
interface FinanceStoreState extends AppState {
  ...
  getNetProfitStats: () => { faturamentoBruto: number, lucroRealAtual: number, pilarX: number, pilarY: number, pilarB: number, faturamentoAlvo: number, deficitAcumulado: number };
    ...
    (set, get) => ({
      ...
      getNetProfitStats: () => {
        ...
        return {
          ...
          faturamentoAlvo,
          deficitAcumulado: state.deficitAcumulado || 0
        };
      },
      ...
      applyFinancialImpact: (impact) => set(state => {
        ...
        const processImpact = (val: number, isAmortization = false, canPayDeficit = true) => {
          let res = val;
          if (res < 0) {
            deficitDelta += Math.abs(res);
            res = 0;
          } else if (res > 0 && currentDeficit > 0 && canPayDeficit) {
        ...
        newCombustivel = processImpact(newCombustivel, false, false);
        newManutencao = processImpact(newManutencao, false, false);
        newMetaSobrevivencia = processImpact(newMetaSobrevivencia, false, true);
        newSurplusReserve = processImpact(newSurplusReserve, false, true);
        newSubsistenceReserve = processImpact(newSubsistenceReserve, false, false);
        
        newSaldosCombustivel = processImpact(newSaldosCombustivel, false, false);
        newSaldosManutencao = processImpact(newSaldosManutencao, false, false);
        newSaldosSurplus = processImpact(newSaldosSurplus, false, true);
      ...
      addRide: (rideData: Ride, manualOdo?: number) => {
        const state = get();
        // Use only displacement and productive KM for ride reserves. 
        // kmEmpty is handled by incrementOdo during 'idle' stage.
        const kmRide = (rideData.telemetry.kmDisplacement || 0) + (rideData.telemetry.kmProductive || 0);
        const kmTotal = rideData.telemetry.kmTotal; // Still keep total for telemetry
        ...
        const operationalCost = kmRide * get().getCostPerKm();
        const maintReserve = kmRide * get().getMaintReservePerKm();
        ...
        // --- HIERARQUIA DE PRIORIDADE (WATERFALL) ---
        const valueNet = rideData.valueGross - fee_platform - fee_gateway;
        let remaining = valueNet;

        // 1º Combustível (Daily Meta)
        const fuelAllocated = Math.min(remaining, operationalCost);
        remaining -= fuelAllocated;

        // 2º Manutenção (Proporcional)
        const maintAllocated = Math.min(remaining, maintReserve);
        remaining -= maintAllocated;

        // 3º Custos Fixos (Divisão Diária)
        const todayStr = new Date().toDateString();
        const todayRides = state.rides.filter(r => new Date(r.timestamp).toDateString() === todayStr);
        ...
        const subsistenceAllocated = Math.min(remaining, subsistenceNeeded);
        remaining -= subsistenceAllocated;

        // 4º Lucro Real (Sobra)
        const netProfit = Math.max(0, remaining);
        ...
        const newRide: Ride = {
          ...
          valueNet: valueNet,
          valuePocket: netProfit,
          valueOperational: fuelAllocated + maintAllocated,
          reservesApplied: {
            salaryFixed: netProfit,
            fuel: fuelAllocated,
            maintenance: maintAllocated,
            amortization: subsistenceAllocated, 
          }
        };
        ...
        // Aplica os impactos financeiros seguindo a lógica de déficit
        // O impacto é negativo se o valor alocado não cobriu o custo/meta
        get().applyFinancialImpact({
          combustivel: fuelAllocated - operationalCost,
          manutencao: maintAllocated - maintReserve,
          subsistenceReserve: subsistenceAllocated - subsistenceNeeded,
          metaSobrevivencia: netProfit
        });
      },
